...
- Tenant public job posting lives at `/post-a-job`. It creates draft jobs first; the payment/publishing step comes later. The form asks for job title, location, job type, job description, contact first name, contact last name, phone, and email. It can optionally create a tenant-scoped employer account in a separate account block below the contact details.
...
- The tenant `/post-a-job` add-job form uses a Quill rich text editor for the job description. Rich text HTML must be sanitized server-side before saving and rendered with `.rich-text`.
